...
class ExampleParser(argparse.ArgumentParser):
  def __init__(self):
    super(ExampleParser).__init__(parents=[
    ...
    self.add_argument(
      "--application_specific_arg", "-asa",
      help="This arg is not part of the generic arg offerings."
    )
...
class LocationParser(argparse.ArgumentParser):
  """Default parser for specifying file locations.
...
class DeviceParser(argparse.ArgumentParser):
  """Default parser for specifying compute devices (WIP).
  ...
  def __init__(self, add_help=False, multi_gpu=False):
    super(DeviceParser, self).__init__(add_help=add_help)
...
class SupervisedParser(argparse.ArgumentParser):
  """Default parser for specifying common supervised learning parameters.
  ...
  def __init__(self, add_help=False, train_epochs=False, epochs_per_eval=False,
               batch_size=False):
    ...
    if num_parallel_calls:
      self.add_argument(
          ...
          help="[default: %(default)s] The number of records that are "
               ...
               "optimized per data set but for generally homogeneous data sets,"
               " should be approximately the number of available CPU cores.",
          metavar="<NPC>"
      )
...
class DummyParser(argparse.ArgumentParser):
  ...
  Args:
    add_help: Create the "--help" flag. False if class instance is a parent.
    use_synthetic_data: Create a flag to use fake data for performance testing.
  """

  def __init__(self, add_help=False, use_synthetic_data=False):
